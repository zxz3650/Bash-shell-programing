...
입력과 출력을 연결하는 방식도 프로그램 계약이다. stdout에는 결과 데이터, stderr에는 진단을 출력하면 결과 파일을 다른 명령이 안정적으로 읽을 수 있다. 저장 도중 실패해도 기존 결과를 보존하려면 임시 파일을 거친다.
