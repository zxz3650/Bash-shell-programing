...
정상 입력은 true와 상태 0이다. -e는 마지막 출력이 false 또는 null이면 0이 아닌 종료 상태를 만든다. 문법 오류나 타입 오류도 실패하지만 원인은 다르므로 stderr를 확인한다. 빈 배열도 유효한 입력으로 처리할지 요구사항에 명시한다.
