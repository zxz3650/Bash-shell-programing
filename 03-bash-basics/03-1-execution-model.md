...
셸 프로그래밍의 기본 단위는 명령이다. Bash는 입력을 읽고 확장한 뒤 명령과 인수를 구분하여 실행한다. 명령을 이해하려면 어떤 인수를 받는지, 어디에 결과와 오류를 출력하는지, 어떤 종료 상태를 반환하는지 확인해야 한다.
...
0은 성공이다. 1 이상은 명령별 의미를 확인해야 한다. grep의 1은 검색 결과 없음, 2는 오류다. 0이 아닌 모든 종료 상태를 같은 장애로 취급하면 빈 결과를 잘못 판단한다.
