...
### 사례: 같은 문자열은 같은 대상을 뜻하는가

**Red Team 질문:** 시스템·사용자·서비스를 식별하는 정보가 필요한 사람에게만 공개되는가? 문자열의 발견은 정보 노출 검토의 출발점이지만, 해당 정보로 다른 자원에 접근할 수 있다는 결론은 아닙니다.

| 연결 단계 | 분석 내용 |
|---|---|
| Goal / Boundary | 정보를 통해 환경을 이해하려는 목적과 파일·로그 읽기 권한의 경계 |
| Command / Observation | ioc-search.sh·grep -F·awk 필드 비교. literal=1과 regex=2는 서로 다른 검색 의미 |
| System Change / Artifact | 검색 결과는 분석가가 만든 파생 자료. 원문 위치·검색 조건·수행 시각 보존 |
| Log prerequisite | 사전 파일 접근 감사가 있어야 읽기 흔적을 기대할 수 있음. 일반 auth.log는 검색 내용을 보장하지 않음 |
| Blue Team Investigation | 일치한 원문 필드가 주소·계정·주석 중 무엇인지, 출처와 기간이 맞는지 확인 |
| Detection | 정확한 필드 일치와 사건 문맥을 결합. 부분 문자열 단독 일치는 우선 검토 후보 |
| Mitigation | 필요 이상의 정보 공개를 줄이고 수집 사본·검색 결과의 접근 권한을 제한 |

**반례와 해설:** 문서에 적힌 예시 도메인은 실제 통신 이력이 아닙니다. substring=2, exact_field=1이라면 두 건을 IOC 통신 두 건으로 보고할 수 없습니다. 정규식 메타문자가 검색 의미를 바꾸는 오류와 공격자 행위는 별개입니다.

**제출 과제:** literal 결과 한 행의 출처·필드·검색 방식을 적고, Red Team의 노출 범위 질문 하나와 Blue Team의 실제 사용 흔적 질문 하나를 작성합니다. “일치 없음”과 “읽기 실패”가 보고서에서 구분되어야 통과입니다.
