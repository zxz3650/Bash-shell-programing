...
변수·배열·인용·조건·반복·함수를 연결하여 입력한 이벤트 수준을 분류한다. 파일과 시스템 정보 없이 메모리 안의 입력부터 검증한다. 이후 같은 입력 검증과 분류 방식을 로그 분석기에 적용한다.

{% hint style="info" %}
### 🧭 학습 목표

- 요구사항을 바탕으로 각 함수의 입력·출력과 오류 처리 방식을 정한다.
- 여러 인수의 경계를 보존한다.
...
열 사이는 공백이 아니라 탭으로 구분한다. 화면에서 비슷하게 보여도 실제 구분 문자가 다르면 프로그램이 열을 잘못 읽을 수 있다.
...
DEBUG를 허용하려면 함수의 case, 요구사항 표, 테스트를 함께 바꾼다. 모든 소문자를 자동 변환할지 엄격하게 거부할지도 먼저 정한다. 코드만 바꾸고 요구사항과 테스트를 수정하지 않으면 어떤 결과가 올바른지 판단하기 어렵다.
...
- [ ] 각 함수가 입력을 처리하고 결과 또는 오류를 반환하는 방식을 설명한다.
