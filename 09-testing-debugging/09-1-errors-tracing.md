...
| 옵션 | 기능 | 한계 |
|---|---|---|
| -e | 일부 예외 상황을 제외하고 명령 실패 시 종료 | if·논리 목록 등 예외 있음 |
| -u | 미설정 변수 확장을 오류 처리 | 빈 값 검증을 대신하지 않음 |
| pipefail | 파이프 앞 단계 실패도 상태에 반영 | 검색 결과가 없을 때도 0이 아닌 상태 반환 가능 |
...
이제 failure만 출력된다. 파일 수집·압축·결과 저장처럼 필수 단계에서는 명시적 상태 전달을 사용한다.
