...
09·10·11은 오류 처리·모듈화·병렬 처리를 다루는 기본 교안의 심화 실습입니다.
...
07장 `security-07-permissions.ipynb`에는 파일 접근 허용·거부, 사본 무결성, 인코딩, 권한 메타데이터, 합성 이벤트 분석의 GTFOBins 확장 실험이 포함됩니다. **Ubuntu 일반 사용자 환경**에서 실행합니다. root로 실행되는 Colab과 macOS는 이 확장 실험의 기준 환경이 아닙니다. sudo 정책이나 특수 권한은 변경하지 않습니다.

기본 노트북의 문법·실행 과제를 마친 뒤 같은 주제의 security 노트북을 진행합니다. security-03부터 security-12까지는 Red Team ↔ Blue Team 사례와 분석 기록도 작성합니다. 07장의 GTFOBins 카드는 별도 합성 훈련이고 12장의 DFIR 사건 자료와 구분합니다.
