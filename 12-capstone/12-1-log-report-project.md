...
지금까지의 문법·파일·텍스트·오류 처리를 하나의 명령줄 도구로 연결한다. 제공된 합성 로그를 검증하고 수준별 개수를 TSV 보고서로 생성한다. 목적은 명령을 많이 사용하는 것이 아니라 입력부터 결과 게시까지 계약을 지키는 것이다.

{% hint style="info" %}
### 🧭 프로젝트 목표

- --input·--output·--dry-run·--help를 지원한다.
- 잘못된 입력에서 결과를 게시하지 않는다.
- 기존 파일과 원본을 보존한다.
...
5. 검사 후 충돌에도 기존 결과를 덮지 않는 게시 방식을 선택한다.
...
기준 구현은 같은 디렉터리의 임시 파일을 만든 뒤 하드 링크로 최종 이름을 게시한다. 최종 경로가 이미 있으면 링크 생성이 실패하여 덮어쓰지 않는다. 이 방식을 지원하지 않는 파일시스템에서는 실패하도록 설계했다. Linux/macOS의 일반 로컬 파일시스템에서 실습한다.
...
서비스별 집계, JSON 출력, 수준 allowlist 설정 중 하나를 추가한다. 입력·출력 계약과 테스트를 먼저 바꾼다. 현재 구현은 인증 로그 일반 파서가 아니므로 실제 로그를 넣으려면 그 형식의 명세와 오류 정책이 필요하다.

## ✅ 제출 기준

- [ ] README에 계약·사용법·플랫폼·한계가 있다.
- [ ] 고정 정답과 결과가 일치한다.
