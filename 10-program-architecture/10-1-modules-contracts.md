# 10-1. 모듈 구성과 함수의 입력·출력

스크립트가 커지면 인수 처리, 데이터 처리, 파일 저장 코드가 뒤섞이기 쉽다. 역할에 따라 함수를 나누고, 각 함수가 받는 입력과 변경할 수 있는 파일·변수를 명확히 정하면 기능별로 테스트할 수 있다.
...
함수가 결과를 반환하는 것 외에 파일이나 전역 변수를 변경하는 동작을 **부수 효과(side effect)**라고 한다. 파일 저장처럼 필요한 부수 효과는 어떤 함수가 담당하는지 명시한다.

| 구성 | 입력 | 출력 | 부수 효과 |
|---|---|---|---|
| 옵션 해석 | argv | 설정 변수 | 없음 |
| summarize_events | stdin 레코드 | 검증된 TSV | 없음 |
| 결과 저장 | 임시·최종 경로 | 성공·실패 | 새 결과 파일 |
...
- [ ] 각 함수의 입력 인수, stdout·stderr 출력, 종료 상태가 문서화되어 있다.
