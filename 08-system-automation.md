...
환경 변수와 옵션 중 무엇이 우선인가, 도움말과 사용 오류는 같은 상태인가, 작업이 겹치면 어떻게 되는가를 먼저 생각합니다. 노트북 07로 자동화 작업의 성공·실패 처리를 확인한 뒤 Linux 환경에서 예약·잠금 설정을 학습합니다. 스케줄을 실제 등록하는 것과 설정 예제를 읽는 것을 구분합니다.
